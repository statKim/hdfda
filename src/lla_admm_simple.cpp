 ...
 */
VectorXd admm_solver_simple(const Eigen::MatrixXd& Sigma,
...
// [[Rcpp::export]]
Rcpp::List group_lla_admm_simple(const Eigen::MatrixXd& Sigma,
                                 const Eigen::VectorXd& delta,
                                 int s, // group size
                                 double lambda = 0.1,
                                 double gamma = 3.7,
  ...
  for (iter = 0; iter < max_iter_lla; ++iter) {
    ...
    beta_hat = admm_solver_simple(Sigma, delta, group_weights, s, rho,
