...
Rcpp::List group_scad_flr_cpp(const Eigen::Map<Eigen::VectorXd> Y,
  ...
  // Pre-calculate the pseudo-inverse part (Theta_j' * Theta_j)^-1 * Theta_j' for each group.
  // This avoids redundant, expensive calculations inside the main loop.
  std::vector<Eigen::MatrixXd> pseudo_inv_list(p);
  for (int j = 0; j < p; ++j) {
    ...
    pseudo_inv_list[j] = Tjt_Tj.ldlt().solve(Theta_j.transpose());
  }
  ...
  for (iter = 0; iter < max_iter; ++iter) {
    ...
    for (int j = 0; j < p; ++j) {
      ...
      const auto& Theta_j = Theta.block(0, j * s, n, s);
      const auto& pseudo_inv_j = pseudo_inv_list[j];
      
      // P_hat_j = Theta_j * [(Theta_j' * Theta_j)^-1 * Theta_j' * R_j]
      Eigen::VectorXd P_hat_j = Theta_j * (pseudo_inv_j * R_j);
  ...
  for (int j = 0; j < p; ++j) {
    const auto& pseudo_inv_j = pseudo_inv_list[j];
    eta_hat_j.col(j) = (pseudo_inv_j * f_hat.col(j));
  }
